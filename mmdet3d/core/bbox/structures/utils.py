...
def xyzwhlr2xyzxyzr(boxes_xyzwhlr):
    """Convert a rotated boxes in XYZWHLR format to XYZXYZR format.
    ...
    Returns:
        torch.Tensor: Converted boxes in XYXYR format.
    """
    boxes = torch.zeros_like(boxes_xyzwhlr)
    half_w = boxes_xyzwhlr[:, 3] / 2
    half_h = boxes_xyzwhlr[:, 4] / 2
    half_l = boxes_xyzwhlr[:, 5] / 2

    boxes[:, 0] = boxes_xyzwhlr[:, 0] - half_w
    boxes[:, 1] = boxes_xyzwhlr[:, 1] - half_h
    boxes[:, 2] = boxes_xyzwhlr[:, 2] - half_l
    boxes[:, 3] = boxes_xyzwhlr[:, 0] + half_w
    boxes[:, 4] = boxes_xyzwhlr[:, 1] + half_h
    boxes[:, 5] = boxes_xyzwhlr[:, 2] + half_l
    boxes[:,6:] = boxes_xyzwhlr[:, 6:]
    return boxes
