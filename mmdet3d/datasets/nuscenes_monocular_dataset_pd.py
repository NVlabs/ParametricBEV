...
class NuScenesMultiViewDataset_PD(MultiViewMixin, NuScenesDataset):
    ...
    def get_data_info(self, index):
        ...
        data_info['img_prefix'] = [None] * n_cameras
        data_info['img_info'] = [dict(filename=x) for x in data_info['img_filename']]
        data_info['lidar2img'] = dict(
                extrinsic=[x.astype(np.float32) for x in data_info['lidar2img']],  # THIS IS *WRONG*. lidar2img is KRT, lidar2cam is RT. 
                intrinsic=np.eye(4, dtype=np.float32) # ???
            )
        data_info['lidar2cam'] = [x.astype(np.float32) for x in data_info['lidar2cam']]
        data_info['cam2img'] = [x.astype(np.float32) for x in data_info['cam2img']]
    ...
    def show(self, results, out_dir='trash', bev_seg_results=None, thr=0.3):
        ...
        for i, result in enumerate(results):
            ...
            for idx in range(len(labels)):
                bev_pred_img = self.draw_bev_bbox_corner(bev_pred_img, bev_box_pred[idx],
                                                        colors[labels[idx]], scale_fac)
            ...
            if bev_seg_results is not None:
                ...
                all_bev_gt_road.append(bev_gt_road)
                bev_seg_gt = map2lssmap(bev_gt_road, bev_gt_lane)
                bev_gt_img = mmcv.imresize(bev_seg_gt,
                                             (100*scale_fac, 100*scale_fac),
                                             interpolation='bilinear')
            ...
            if len(labels_gt) > 0:
                ...
                for idx in range(len(labels_gt)):
                    bev_gt_img = self.draw_bev_bbox_corner(bev_gt_img, bev_gt_bboxes[idx],
                                                        colors[labels_gt[idx]], scale_fac)
            ...
            for j in range(len(info['img_info'])):
                ...
                # draw GT
                if len(gt_bboxes['gt_labels_3d']) > 0:
        ...
        ###### Generate bev gt road heatmap ######
        sum_bev_gt_road = np.stack(all_bev_gt_road).sum(axis=0)
        prob_bev_gt_road = sum_bev_gt_road/len(all_bev_gt_road)
        plt.imshow(prob_bev_gt_road, cmap='hot')
        plt.colorbar()
        plt.grid(None)
        plt.show()
            
        ###### generate videos step:2 ######
        gen_video(all_img_pred, all_bev_pred, out_dir, 'pred')
        gen_video(all_img_gt, all_bev_gt, out_dir, 'gt')
        ###### generate videos step:2 ######
...
def gen_video(img_list, bev_list, out_dir, mode='pred', fps=3):
    if mode == 'pred':
        out_video_path = os.path.join(out_dir, 'video_pred.mp4', fps=3)
    else:
        assert mode == 'gt'
        out_video_path = os.path.join(out_dir, 'video_gt.mp4',fps=3)
    tmp_list = []
...
def draw_ego_car(bev):
    ...
    ego_h = 8
    ego_w = 20
    x1 = (h - ego_h) // 2; y1 = (w - ego_w) // 2
    x2 = (h + ego_h) // 2; y2 = (w + ego_w) // 2
    color = [255,0,0]
