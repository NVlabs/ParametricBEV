...
class DeformableDETRHead3DDebug3(DETRHead):
    ...
    def init_weights(self):
        ...
        # nn.init.constant_(self.reg_branches[0][-3].bias.data[:2], -2.0)
        # nn.init.constant_(self.reg_branches[0][-3].bias.data[:2], -3.0)
        nn.init.constant_(self.reg_branches[0][-3].bias.data[:2], -4.0)
    ...
    def forward(self, mlvl_feats):
        ...
        return outputs_classes, outputs_coords, outputs_dirs, init_reference
    ...
    def loss(self,
        ...
        '''
        losses_cls, losses_bbox_l1, losses_bbox_iou, sampling_results = multi_apply(
        ...
        # debug_vis = False if world_size > 1 else True
        debug_vis = True
        if debug_vis and rank == 0:
            ...
            def draw_bbox(img, bbox_pred, bbox_gt, scores):
                color1 = (0, 0, 255)
                color2 = (0, 255, 0)
                color_line = (255, 255, 255)
                for box_pred, box_gt, score in zip(bbox_pred, bbox_gt, scores):
                    img = cv2.rectangle(img, (box_gt[0], box_gt[1]), (box_gt[2], box_gt[3]), color1, 1)
                    img = cv2.rectangle(img, (box_pred[0], box_pred[1]), (box_pred[2], box_pred[3]), color2, 1)
                    img = cv2.putText(img, "{:.2f}".format(score.item()), (box_pred[0], box_pred[1]-2),
                                      cv2.FONT_HERSHEY_SIMPLEX, 0.3, color2, 1)
                    img = cv2.line(img, ((box_gt[0]+box_gt[2])/2, (box_gt[1]+box_gt[3])/2),
                                        ((box_pred[0]+box_pred[2])/2, (box_pred[1]+box_pred[3])/2),
                                         color_line, 1)
                return img  
            vis_layer = -1
            vis_gt_bboxes = all_gt_bboxes_list[vis_layer][0].bev[:,:4] 
            vis_gt_bboxes = bbox_convert(vis_gt_bboxes)
            ###
            vis_pred_bboxes = all_bbox_preds[vis_layer][0,:,[0,1,3,4]]
            vis_pred_bboxes[:,:2]*=100; vis_pred_bboxes[:,2:]*=20; vis_pred_bboxes = bbox_cxcywh_to_xyxy(vis_pred_bboxes)
            if self.loss_cls.use_sigmoid:
                vis_scores = all_cls_scores[vis_layer][0].sigmoid().max(-1)[0]
            else:
                vis_scores = F.softmax(all_cls_scores[vis_layer][0], dim=-1)[..., :-1].max(-1)[0]
            
            # 选出pred和gt match的
            sampler = sampling_results[vis_layer][0]
            vis_gt_bboxes = vis_gt_bboxes[sampler.pos_assigned_gt_inds]
            vis_pred_bboxes = vis_pred_bboxes[sampler.pos_inds]
            vis_scores = vis_scores[sampler.pos_inds]
            # draw bbox
            resize_scale = 5
            img = np.zeros((100*resize_scale,100*resize_scale,3),dtype='uint8')
            #中心是ego car
            cv2.circle(img, (50*resize_scale,50*resize_scale), 5, (255,255,255),-1)
            vis_gt_bboxes *= resize_scale; vis_pred_bboxes *= resize_scale
            img_dir = 'trash/{}'.format(img_metas[0]['sample_idx'])
            img = draw_bbox(img,vis_pred_bboxes, vis_gt_bboxes, vis_scores)
            mmcv.imwrite(img, '{}/matching.png'.format(img_dir), auto_mkdir=True)
            print('vis matching')
            # embed(header='vis matching ')
        ...
        num_dec_layer = 0
        for loss_cls_i, loss_bbox_l1_i, loss_bbox_iou_i in zip(losses_cls[:-1],
                                              losses_bbox_l1[:-1],
                                              losses_bbox_iou[:-1]):
    ...
    def loss_single(self,
        ...
        # regression L1 loss
        '''对于loss_bbox 是对gt和pred归一化 算l1'''
        '''debug 只考虑xy wh'''
        bbox_preds = bbox_preds[:, [0,1,3,4]]
        bbox_targets = bbox_targets[:, [0,1,3,4]]
        bbox_weights = bbox_weights[:, [0,1,3,4]]
        factor_xyz_whl_r_vxvy = bbox_targets.new_tensor([100,100,8,20,20,20,0,0,0])
        factor_xyz_whl_r_vxvy = factor_xyz_whl_r_vxvy[[0,1,3,4]]
        
        norm_bbox_pred = bbox_cxcywh_to_xyxy(bbox_preds*factor_xyz_whl_r_vxvy) / factor_xyz_whl_r_vxvy[0]
        norm_gt_bboxes = bbox_cxcywh_to_xyxy(bbox_targets*factor_xyz_whl_r_vxvy) / factor_xyz_whl_r_vxvy[0]
        loss_bbox = self.loss_bbox(norm_bbox_pred*bbox_weights,
                                   norm_gt_bboxes*bbox_weights, 
                                   bbox_weights,
                                   avg_factor=num_total_pos)
        
        # 计算iou loss 把xywh缩放回原始的
        real_bbox_pred = bbox_cxcywh_to_xyxy(bbox_preds * factor_xyz_whl_r_vxvy)
        real_gt_bboxes = bbox_cxcywh_to_xyxy(bbox_targets * factor_xyz_whl_r_vxvy)
        loss_iou = self.loss_iou(real_bbox_pred,
                                 real_gt_bboxes,
                                 bbox_weights,
                                 avg_factor=num_total_pos)
        return loss_cls, loss_bbox, loss_iou, sampling_results
    ...
    def get_targets(self,
        ...
        (labels_list, label_weights_list,
         bbox_targets_list, bbox_weights_list,
         dir_targets_list, dir_weights_list,
         pos_inds_list, neg_inds_list, sampling_results) = multi_apply(
        ...
        return (labels_list, label_weights_list, 
                bbox_targets_list, bbox_weights_list,
                dir_targets_list, dir_weights_list,
                num_total_pos, num_total_neg, sampling_results)
    ...
    def _get_bboxes_single(self,
        ...
        return det_bboxes, scores, det_labels, ref_points
