...
class FreeAnchor3DCtnessHead(Anchor3DHead):
    ...
    def loss(self,
        ...
        ### bev centerness ###
        gt_bboxes_center = gt_bboxes[0].bev[:,:2].to(positive_losses[0].device)
        max_dist = 70.7 # 50 x 2.sqrt()
        dist = (gt_bboxes_center[:,0]**2+gt_bboxes_center[:,1]**2).sqrt()
        pos_centerness = dist / max_dist + 1
        positive_losses = torch.cat(positive_losses) * pos_centerness
        ### bev centerness ###
        
        positive_loss = positive_losses.sum() / max(1, num_pos)
        ...
        ''''''
        ### bev centerness ###
        tmp_neg_loss = self.negative_bag_loss(cls_prob, box_prob)
        # TODO negative bev centerness
        ### bev centerness ###
        negative_loss = tmp_neg_loss.sum() / max(
