...
class PDBEV_FCNHead(BEV_BaseDecodeHead):
    ...
    def __init__(self,
        ...
        super(PDBEV_FCNHead, self).__init__(**kwargs)
        if num_convs == 0:
        ...
        self.split_vis_occ_seg_loss = split_vis_occ_seg_loss
        self.seg_vis_region_weight = seg_vis_region_weight
        self.seg_occ_region_weight = seg_occ_region_weight
    ...
    def losses(self, seg_logit, seg_label):
        ...
        loss_dice = 0.5 * (self.loss_dice(seg_pred_road.sigmoid(), seg_label_road, ignore_index=self.ignore_index) + \
                           self.loss_dice(seg_pred_lane.sigmoid(), seg_label_lane, ignore_index=self.ignore_index))
        if not self.use_centerness:
            loss_ce = 0.5 * (self.loss_ce(seg_pred_road, seg_label_road, ignore_index=self.ignore_index) + \
                             self.loss_ce(seg_pred_lane, seg_label_lane, ignore_index=self.ignore_index))
        else:
            self.loss_ce.reduction = 'none'
            tmp_loss = 0.5 * (self.loss_ce(seg_pred_road, seg_label_road, ignore_index=self.ignore_index) + \
                             self.loss_ce(seg_pred_lane, seg_label_lane, ignore_index=self.ignore_index))
...
def iou(pred, tgt):
    ...
    # valid_mask = (tgt!=255) # Hardcode ignore_index to be 255.
