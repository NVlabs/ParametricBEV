...
def main():
    ...
    if args.launcher == 'none':
        ...
        distributed = True
        if "nccl_timeout_s" in cfg.dist_params:
            from datetime import timedelta
            timeout = timedelta(seconds=cfg.dist_params['nccl_timeout_s'])
            del cfg.dist_params['nccl_timeout_s']
            for iii in range(10):
                print("Setting NCCL timeout to (s):",timeout.total_seconds())
        else:
            from datetime import timedelta
            timeout = timedelta(seconds=1800) # Default 30min timeout
            # del cfg.dist_params['nccl_timeout_s']
            for iii in range(10):
                print("Setting NCCL timeout to (s):",timeout.total_seconds())
        init_dist(args.launcher, timeout=timeout, **cfg.dist_params)
